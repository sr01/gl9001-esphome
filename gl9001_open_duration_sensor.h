...
class GL9001OpenDurationSensor : public PollingComponent,
 ...
 protected:
  std::string empty_value = "00:00:00";
  const char *timeFormat = "%02d:%02d:%02d";
  uint16_t irrigationStatusHandle;
  IrrigationStatus prevStatus = {false, false, 0, 0, 0};
  ...
  IrrigationStatus parse_irrigation_status_data(uint8_t *value, uint16_t value_len);
  void readFaucetStatus();
  std::string formatTime(uint8_t hour, uint8_t minute, uint8_t second);
};
